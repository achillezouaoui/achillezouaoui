...
function downloadCV() {
    ...
    link.href = './Achille_Zouaoui_Senior_Product_Designer.pdf';
    link.download = 'Achille_Zouaoui_Senior_Product_Designer.pdf';
...
// ── Redirect to Portfolio when scrolling past the Contact section (desktop only) ──
if (!isMobile) {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        window.addEventListener('wheel', (e) => {
            ...
            if (e.deltaY > 0 && atBottom()) {
                scrollIntent += e.deltaY;
                clearTimeout(intentTimer);
                // Reset intent if user stops scrolling for 800ms
                intentTimer = setTimeout(() => { scrollIntent = 0; }, 800);
                // Only redirect after 250px of accumulated scroll delta
                if (scrollIntent >= 250) goToPortfolio();
            } else {
                scrollIntent = 0;
                clearTimeout(intentTimer);
            }
